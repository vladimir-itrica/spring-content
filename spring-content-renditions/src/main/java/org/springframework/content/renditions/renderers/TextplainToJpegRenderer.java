...
public class TextplainToJpegRenderer implements RenditionProvider {
    ...
    public InputStream convert(InputStream fromInputSource, String toMimeType) {
        ...
        if (lineCnt != 0) {
            try {
                File saveLocation = new File(FileUtils.getTempDirectoryPath(), "textToJpeg.jpeg");
                saveImage(images, lineHeight, saveLocation.getPath());
                return new FileInputStream(saveLocation);
            } catch (IOException e) {
    ...
    private void saveImage(ArrayList<BufferedImage> images, int lineHeight,
        ...
        for (int i = 0; i < images.size(); i++) {
            g.drawImage(images.get(i), 0, margin + (i * lineHeight),
                    null);
        }
